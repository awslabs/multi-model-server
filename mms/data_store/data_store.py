...
class DataStore:
    ...
    def get(self, id, data_type, timeout=60):
        codec = self._get_codec(data_type)
        return codec.deserialize(self.hashmap.get(id, timeout))
    ...
    def set_batch(self, ids, data, data_type):
        ...
        for id, datum in zip(ids, data):
            self.hashmap.put(id, codec.serialize(datum))
    ...
    def _get_codec(data_type):
        if data_type == "image/jpeg":
            ...
            # This should never happen, data type is checked upstream
            return
