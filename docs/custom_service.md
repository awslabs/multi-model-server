...
```handle()``` method itself. The ```initialize()``` method is used to initialize the model at load time, so after first time, the service need not be re-initialized in the the life cycle of the relevant worker.
 ...
 This entry point is engaged in two cases: (1) when MMS is asked to scale model up or down (it is done either via a ```PUT /models``` request or a ```POST /models``` request with `initial-workers` option or if during MMS startup (```mxnet-model-server --start```) you provide model(s) to load) or (2) when MMS gets ```POST /predictions``` request. (1) is used to scale up/down workers for model that has been already registered, e.g. scale model up or down. (2) is used as a standard way to run inference against models. (1) is also known as model load time, and that is where you would normally want to put code for model initialization. You can find out mode about these and other MMS APIs in [MMS Management API](./management_api.md).
